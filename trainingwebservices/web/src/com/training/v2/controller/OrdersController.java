...
import com.training.strategies.OrderCodeIdentificationStrategy;
import com.training.v2.helper.OrdersHelper;
...
public class OrdersController extends BaseCommerceController
{
	...
	@RequestParam(defaultValue = DEFAULT_FIELD_SET)
	final String fields) throws PaymentAuthorizationException, InvalidCartException, NoCheckoutCartException, Exception
	{
		LOG.info("placeOrder");
		...
		//		List<OrderHistoryData> order= orderFacade.getOrderHistoryForStatuses(OrderStatus.CREATED);
		//		final OrderStatus orderStatus = null;
		final List<OrderHistoryData> orders = orderFacade.getOrderHistoryForStatuses(OrderStatus.CREATED);

		if (CollectionUtils.isNotEmpty(orders))
		{
			if (OrderStatus.CREATED.getCode().equals(orders.get(0).getStatus().getCode()))
			{
				throw new Exception("already you have one order");
			}
		}
